...
func InsertHistory(method, url, headers, params, bodyType, body string, responseStatus int) (HistoryEntry, error) {
	res, err := DB.Exec(`
		INSERT INTO request_history (method, url, headers, params, body_type, body, response_status, executed_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
	`, method, url, headers, params, bodyType, body, responseStatus)
	if err != nil {
...
func GetHistoryEntry(id int64) (HistoryEntry, error) {
	row := DB.QueryRow(`
		SELECT id, method, url, headers, params, body_type, body, response_status, executed_at
		FROM request_history WHERE id = ?
	`, id)
	var e HistoryEntry
	if err := row.Scan(&e.ID, &e.Method, &e.URL, &e.Headers, &e.Params, &e.BodyType, &e.Body, &e.ResponseStatus, &e.ExecutedAt); err != nil {
...
func ListHistory(limit int) ([]HistoryEntry, error) {
	...
	for rows.Next() {
		var e HistoryEntry
		if err := rows.Scan(&e.ID, &e.Method, &e.URL, &e.Headers, &e.Params, &e.BodyType, &e.Body, &e.ResponseStatus, &e.ExecutedAt); err != nil {
